...
int ControlPlay(AudioOutput_v1* user,MAVFrame* audio_frame){
    ...
    //太慢了(丢帧)或者frame不属于该序列
    if(sleepMs<-40||audio_frame->serial!=user->p_state->GetSerial()){
        return 0;
    }
...
int Resample(AudioOutput_v1* user,AVFrame* frame,Uint8** out_data,int* audio_buf_size)
{
    ...
    int outSamples=(inSamples*user->obtained_para.spec_.freq)/(user->desired_para.spec_.freq/user->p_state->pSpeed)+256;
    //计算输出缓冲区的总大小
...
void  audioCallBack4(void *userdata, Uint8 * stream,int len)
{
    ...
    while(len>0){
        ...
        //printf("Audio Frame pts=%d;SyncTime pts=%d\n",mFrame->frame->pts,user->sync_clocker->getSyncDrift());
...
int AudioOutput_v1::BuildSwrContext()
{
    ...
    res=av_opt_set_int(swr_ctx,"out_sample_rate",obtained_para.spec_.freq/p_state->pSpeed,0);
    //AVSampleFormat
