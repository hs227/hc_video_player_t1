...
PlayerState::PlayerState()
    :isPaused(false),stepOne(0),volume_flag(false),
    playPos_flag(0),serial(0),
...
void PlayerState::PlayPosChange(int64_t pos_dif)
{
    pos_dif*=1000;//s->ms
    int64_t newPos=sync_clocker->getSyncDrift();//ms
    newPos+=pos_dif;
    this->SetPosFlag(pos_dif);
    playPosition=newPos;
}
