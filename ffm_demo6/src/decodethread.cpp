...
int DecodeThread::Init(AVCodecParameters *para_,int stream_id_,AVRational timebase_)
{
...
int DecodeThread::Start()
{
    if(this->paras==NULL){
        //没有对应流->必然是字幕流
        if(this->p_state->libass_sys->type!=LibassUser::LU_Type::LU_IN_S){
            return 0;//只有内封字幕需要解码
        }
        return -1;
    }
        ...
        if(pkt){
            ...
            for(;;){
                ...
                if(res==0){
                    // if(posNewPTS>0){
                    //     //有延迟时间
                    //     int64_t srcPts=frame->pts*av_q2d(this->timebase)*1000;//ms
                    //     //int offset=posNewPTS-srcPts;
                    //     int offset=(srcPts/p_state->GetSpeed())-posNewPTS;
                    //     if(offset<-20){
                    //         //延迟过大，不放入
                    ...
                    loops++;
                    //printf("times(%d):%s Frame(size=%d)\n",loops,codec_ctx->codec->name,frame_queue->Size());
