...
int ControlPlay(AudioOutput_v1* user,AVFrame* audio_frame){
    int64_t srcPts=audio_frame->pts*av_q2d(user->time_base)*1000;//ms
    int offset=50;//移位
    int speed=1;//变速
...
void  audioCallBack4(void *userdata, Uint8 * stream,int len)
{
    ...
    while(len>0){
        ...
        frame=user->frame_queue->Front();
        if(!frame){
            //没收到frame
            break;
        }
        //时钟开始控制
        //printf("Audio Frame pts=%d;SyncTime pts=%d\n",frame->pts,user->sync_clocker->getSyncDrift());
        int ret=ControlPlay(user,frame);
        if(ret==0){
            //丢帧
