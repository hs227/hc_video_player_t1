...
int DemuxThread::Init(const char *url_)
{
    ...
    //返回基础信息
    p_state->sync_clocker->SetDuration(ifmt_ctx->duration/(double)AV_TIME_BASE);//单位s
    ...
    while(this->abort!=1){
        ...
        if(p_state->GetPosFlag()!=0){
            ...
            if(res<0){
               ...
               // int tmp2=p_state->GetPlayPos();
               // float tmp3=p_state->GetSpeed();
                p_state->sync_clocker->posChangeSync(p_state->GetPlayPos()/p_state->GetSpeed());
