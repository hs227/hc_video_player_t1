...
AudioOutput_v2::AudioOutput_v2(AVFrameQueue* frame_q,AVRational time_base_,PlayerState* p_s)
    :frame_queue(frame_q),deviceId(0),time_base(time_base_),p_state(p_s)
{
    sync_clocker=p_state->sync_clocker;
}

AudioOutput_v2::~AudioOutput_v2()
{

}

int ControlPlay(AudioOutput_v2* user,MAVFrame* audio_frame){
...
int Resample(AudioOutput_v2* user,AVFrame* frame,Uint8** out_data,int* audio_buf_size)
{
    //只负责格式转换
...
int STPrepare(AudioOutput_v2* user,Uint8** in_data,uint8_t* out_data)
{
    //1.初始化ST(move to init)(实现音频变速)
    if(user->p_state->GetSpeedFlag()==true){
        user->BuildSoundTouch();
    ...
    user->st.putSamples(samples,1);

    // produce the sample out
    user->st.receiveSamples((soundtouch::SAMPLETYPE*)out_data,1);
...
{
    AudioOutput_v2* user=(AudioOutput_v2*)userdata;
    ...
    while(len>0){
        ...
        //再通过soundtouch变速
        uint8_t st_data[8192]={0};
        STPrepare(user,resample_data,st_data);

        //packet格式(重采样后变成了packet格式)
        each_len=resample_data_len;
        if(each_len>len)
            each_len=len;
        memcpy(stream_ptr,st_data,each_len);
...
int AudioOutput_v2::Init(const AVCodecParameters* params)
{
    //1.初始化音频系统
...
void AudioOutput_v2::Play()
{
    //开始音频播放
    SDL_PauseAudioDevice(deviceId,0);
}

void AudioOutput_v2::Pause()
{
    //暂停音频播放
    SDL_PauseAudioDevice(deviceId,1);
}

int AudioOutput_v2::UpdateDevice()
{
    //当播放倍速改变时，需要更新音频设备
...
int AudioOutput_v2::BuildSwrCtx()
{
    //由于不使用重采样实现变速，
    //在AOv2中，该函数只用于初始化
...
int AudioOutput_v2::BuildSoundTouch()
{
    st.clear();

    //1.配置变调变速参数
    st.setSampleRate(config.out_sample_rate);//输入采样率
    st.setChannels(config.out_channels);//输入通道数
    //2.设置rate或pitch
    st.setRate(this->p_state->GetSpeed());//设置速度为0.5，原始为1.0
    st.setTempo(0);//不变调
